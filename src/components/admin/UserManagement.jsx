...
const DEPARTMENTS = [
  ...
  "Faculty of Medicine", "Faculty of Computing", "Faculty of Business",
];
...
const AVATAR_COLORS = [
  { bg: "#e5e7eb", color: "#374151" }, { bg: "#1f2937", color: "#f9fafb" },
  { bg: "#dbeafe", color: "#1d4ed8" }, { bg: "#ede9fe", color: "#6d28d9" },
  { bg: "#ffedd5", color: "#c2410c" }, { bg: "#d1fae5", color: "#065f46" },
  { bg: "#fce7f3", color: "#9d174d" }, { bg: "#e0e7ff", color: "#3730a3" },
];
  ...
  return (
    <div className="card h-100 border shadow-sm">
      <div className="card-body ">
        ...
        <h4 className={`fw-bold mb-0 ${badgeColor ? `text-${badgeColor}` : ""}`}>
...
export default function UserManagement() {
  ...
  const allUsers = useMemo(() => [...students, ...admins], [students, admins]);

  const stats = useMemo(() => ({
    total: allUsers.length,
    students: students.filter(u => getUserCategory(u) === "Student").length,
    admins: admins.length,
    faculty: students.filter(u => getUserCategory(u) === "Faculty").length,
    suspended: allUsers.filter(u => u.status === "Suspended").length,
  }), [allUsers, students, admins]);
  ...
  async function handleAddUser() {
    ...
    const targetCol = form.role === "Admin" ? "admins" : "students";
    try {
      await addDoc(collection(db, targetCol), { ...form, createdAt: new Date() });
      setForm(EMPTY_FORM);
      setIsOpen(false);
      setPage(1); // العودة للصفحة الأولى عند الإضافة
    } catch (err) { console.error(err); }
  }

  async function handleDeleteUser(user) {
    if (window.confirm(`Are you sure you want to permanently delete ${user.name}? This cannot be undone.`)) {
      const targetCol = user.role === "Admin" ? "admins" : "students";
      try { await deleteDoc(doc(db, targetCol, user.id)); } catch (err) { console.error(err); }
    }
  }
      ...
      <div className="container-xl px-3">
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 gap-3">
          <div>
            ...
            <p className="text-muted mb-0">Manage access and monitor system activity</p>
          </div>
          <div className="d-flex gap-2">
            <input type="text" className="form-control rounded-4 shadow-sm" placeholder="Search..." value={search} onChange={(e) => {setSearch(e.target.value); setPage(1);}} />
            <button onClick={() => setIsOpen(true)} className="bg-brown text-white px-4 rounded-4 border-0 shadow-sm text-nowrap">+ Add User</button>
          </div>
        </div>

        <div className="row g-3 mb-4">
          <div className="col-6 col-md-3"><StatCard icon="👥" label="Total" value={stats.total} /></div>
          <div className="col-6 col-md-3"><StatCard icon="👮" label="Admins" value={stats.admins} /></div>
            ...
            <ul className="nav nav-tabs card-header-tabs">
              {TABS.map(tab => (
                <li className="nav-item" key={tab}>
                  <button className={`nav-link ${activeTab === tab ? "active fw-bold text-dark" : "text-muted"}`} onClick={() => {setActiveTab(tab); setPage(1);}}>{tab}</button>
                </li>
              ))}
            </ul>
          </div>
                ...
                {visibleUsers.length > 0 ? visibleUsers.map((user) => {
                  ...
                  return (
                    <tr key={user.id}>
                      ...
                      <td><div className="small fw-bold">{user.dept}</div><div className="small text-muted">{user.role}</div></td>
                      <td>{user.books || 0}</td>
                      <td className="text-center">
                        <button 
                          onClick={() => handleToggleStatus(user)}
                          className={`btn btn-sm px-4 rounded-pill fw-bold ${isActive ? "btn-success" : "btn-danger"}`}
                      ...
                      <td>
                        <button className="btn btn-sm text-danger shadow-none" onClick={() => handleDeleteUser(user)}>
                          🗑
                        </button>
                      </td>
                    </tr>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content rounded-4 p-4 border-0 shadow">
              ...
              <div className="row g-3">
                <div className="col-12"><input placeholder="Full Name" className="form-control" value={form.name} onChange={e => setForm({...form, name: e.target.value})} /></div>
                <div className="col-12"><input placeholder="Email" className="form-control" value={form.email} onChange={e => setForm({...form, email: e.target.value})} /></div>
                <div className="col-6">
              ...
              <div className="mt-4 d-flex gap-2 justify-content-end">
                <button className="bg-brown text-white border-0 px-4 py-2 rounded-2" onClick={handleAddUser}>Create</button>
                <button className="btn btn-light border px-4" onClick={() => setIsOpen(false)}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
